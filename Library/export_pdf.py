...
def create_pdfs(client_data):
    ...
    for delivery_date in date_range:
        ...
        # Itera sobre os turnos (manhã e tarde)
        for turno, deliveries in filtered_clients.items():
            pdf.cell(200, 10, txt=f"Turno: {turno.capitalize()}", ln=True, align='L')
            pdf.ln(5)

            # Adiciona informações dos clientes no formato desejado para cada turno
            for index, client in enumerate(deliveries, start=1):
                # Obtendo valores, substituindo valores vazios ou None por "nan"
                numero_pedido = client.get("nº_do_pedido", "nan") or "nan"
                numero_casa = client.get("numero_da_casa", "nan") or "nan"
                entrega_retirada = client.get("entrega_retirada", "nan") or "nan"
                endereco = client.get("endereco_completo", "nan") or "nan"
                modelo_sofa = client.get("modelo_do_sofa", "nan") or "nan"
                ...
                restricao_local = client.get("restricao_de_local", "Sem restrições") or "nan"
                restricao_horario = client.get("restricao_de_horario", "Sem restrições") or "nan"
        ...
        try:
            pdf.output(f"Data/{pdf_file_name}")
